...
`NHMisc` is a Red-DiscordBot V3 cog with small moderation and server-utility tools.
...
Voice logging sends simple log messages when users join, leave, or move between voice
channels. Move logs are sent immediately; when Discord audit logs show that a moderator
moved the member, the bot edits the original move log to include the moderator name and
user ID.
